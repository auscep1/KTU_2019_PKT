...
extern FILE* yyin;
extern pkt2::Block* programBlock;
...
int main(int argc, char **argv)
{
    ...
    if( !quiet )
      std::cout << /*"! pkt2 version " << MAJOR_VER << "." << MINOR_VER << "." << REVISION_VER << */"\n";
    ...
    if( programBlock == nullptr ) {
       ...
       pkt2::CodeGenContext context(quiet ? devNull : std::cout);
...
{
   std::cout << "Usage:\n";
   std::cout << "pkt2 filename -h -d -v -q -i path1;path2\n";
   std::cout << "\t-h this help text.\n";
   std::cout << "\t-d debug code generation. Disables the code optimizer pass.\n";
   std::cout << "\t-v be more verbose.\n";
   std::cout << "\t-q be quiet.\n";
   std::cout << "\t-i semicolon separated list of import paths where additional pkt2 files are located.\n";
}
